...
- **本ガード自身の境界**: 件数ガードは空/切り詰め/破損を捕捉する。内容・ソルト整合（実ソルト下で実コンテンツに
  一致するか）は原理的にCI不能（公開の既知ポジティブ=漏洩）であり、運用保証＝ソルト変更時のテーブル再生成
  （DL-0001 §3 逸脱3）で担保する。
- 教訓: no-op 面は層をなす。1つ塞ぐと「その検出器自身が見ない面」が次の no-op 面になる。
  境界を都度明文化し（DL-0002 §4）、指摘された面を follow-up で閉じる（本DL）という還流で、
  vacuous PASS を段階的に潰す。
- SCR-007 は DL-0002 のカナリア＋本DLのガードで二重に封鎖。クローズ判断は設計レーン（Notion）。
